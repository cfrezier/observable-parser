...
let fnId = 0;
const displayFnExpr = (node) => {
    if (node.parent.arguments) {
        const endOfExpr = node.body.expression.name ? node.body.expression.name.escapedText + "(" + node.body.arguments[0].escapedText + ")" :
            node.body.expression.escapedText + "." + node.body.name.escapedText;
        const preExpr = node.parent.arguments[0].expression.expression.name ? "this." + node.parent.arguments[0].expression.expression.name.escapedText :
            node.parent.arguments[0].expression.expression.escapedText;
        return preExpr + "." +
            node.parent.arguments[0].expression.name.escapedText + "(" +
            node.parameters[0].name.escapedText + " => " + endOfExpr + ")";
    } else {
...
const getName = (node, file) => {
    ...
    if (!!node.parameters && !!node.body) {
        return [displayFnExpr(node)];
    }
    if(ts.SyntaxKind[node.kind] === "ConditionalExpression") {
        return [getName(node.whenTrue, file), getName(node.whenFalse, file)] ;
    }
    ...
    if (recurse.expression) {
        if (!!recurse.expression.name && recurse.expression.name.escapedText === "subscribe") {
            ...
            return (recurse.arguments[0].elements || [recurse.arguments[0].arguments[0]])
                .map(arg => {
                    arg.parent = recurse;
                    return analyseExprTree(file, arg, level + 1)
                })
                .reduce((p, c) => [...p, ...c], []);
        } else {
