...
let BASE_ARROW = "-d->";
let SUBSCRIBE_ARROW = "-u--->";
const asLink = (definitions) => (v, i, a) => {
    const arrow = a[a.length - 1] === "subscribe" ? SUBSCRIBE_ARROW : BASE_ARROW;
    if (isAnObservable(definitions, a)) {
        ...
        if (i < a.length - 1) {
            ...
            definitions.definitions.fromClasses[definitions.packName].links.push("(" + a[a.length - 1] + ") " + arrow + " (" + v + ")");
        }
    }
}
